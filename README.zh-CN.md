...
- 通过 TronLink / TronWeb provider 或 WalletConnect（需 `tronweb`）支持 TRON 钱包。
...
# TRON WalletConnect 付款还需安装 tronweb 用于构造并广播交易：
npm install tronweb
```
...
`solana_signAndSendTransaction`。TRON WalletConnect 需要可选依赖 `tronweb`：SDK 构造未签名的
TRC-20 `transfer`，通过 `tron_signTransaction` 请求钱包签名，再自行广播签名结果。交易在 TRON
全节点上构造/广播——默认使用 trongrid 主网 / Nile，可在调用时用
`sendWalletPayment({ ..., tronRpcUrl })` 覆盖。现有 TronLink / TronWeb 注入路径保持不变；两套
路径并存，按 provider 形状自动选择。
